...
def message_has_photo(comment: Dict[str,Any]) -> bool:
    return any((a.get("content_type","").startswith(("image/","application/pdf"))) 
...
def process_once(client: OpenAI):
    for t in list_recent_tickets():
        # Skippa ticket già chiusi
        if t.get("status") in ("solved","closed"):
            continue
        # Skippa se già messo il tag del draft
        if DRAFT_TAG in (t.get("tags") or []):
            continue

        comments = get_ticket_comments(t["id"])
        # Rispondiamo solo se l’ultimo commento è pubblico e dell’end-user
...
# Con Flask 3 non esiste più before_first_request:
# usiamo before_request (viene invocata a ogni richiesta) ma la funzione
# start_background_once() è idempotente e parte solo la prima volta.
@app.before_request
def _ensure_bg():
...
if __name__ == "__main__":
    # Avvio anche senza richieste (utile su Render free che pinga la root)
    start_background_once()
